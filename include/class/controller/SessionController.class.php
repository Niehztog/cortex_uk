...
class SessionController {
	...
	private function countSessions($expId) {
		
		$dbf = new DatabaseFactory();
		$mysqli = $dbf->get();
		
		$sql = sprintf( '
			SELECT COUNT(1) AS count FROM %1$s WHERE exp = %2$d'
			, TABELLE_SITZUNGEN
			, $expId
		);
		$result = $mysqli->query($sql);
		if(false === $result) {
			trigger_error($mysqli->error, E_USER_WARNING);
			throw new RuntimeException(sprintf('Fehler beim zählen der Sitzungen zu Experiment %1$d.', $expId));
		}
		$data = $result->fetch_assoc();
